-- Barbona Agustina Gabriela agusmua3110@gmail.com
-- Pomi Franco Ezequiel pomifranco@gmail.com
-- Troncoso Sebastian troncoso.seb.99@gmail.com
...
potencia c1 n   | absoluto coseno < 0.00001 && absoluto coseno > 0.00 = (0, r ^ n * seno)
                | absoluto seno < 0.00001 && absoluto seno > 0.00 = (r ^ n * coseno, 0)
...
argumento c1  | re c1 == 0 && im c1 > 0 = (pi) / 2
              | re c1 == 0 && im c1 < 0 = 3 * (pi) / 2
              | re c1 < 0 = (pi) + argI
              | re c1 > 0 && im c1 >= 0 = argI
              | re c1 > 0 && im c1 < 0 = 2 * (pi) + argI
              | otherwise = undefined
              where argI = atan (im c1 / re c1)
...
raicesNEsimasDesde n k  | k == (n-1) = [(parteReal, parteImaginaria)]
                        | otherwise = (parteReal, parteImaginaria) : raicesNEsimasDesde n (k+1)
                        where parteReal = cos ((2 * k * (pi)) / n)
                              parteImaginaria = sin ((2 * k * (pi)) / n)

raicesNEsimas :: Integer -> [Complejo]
raicesNEsimas n = raicesNEsimasDesde (fromInteger n) 0

sonRaicesNEsimas :: Integer -> [Complejo] -> Float -> Bool
sonRaicesNEsimas n l e  | l == [] = True
                        | condicion = sonRaicesNEsimas n (tail l) e
                        | otherwise = False
                        where condicion = (modulo (suma (potencia (head l) n) (-1, 0)) < e)
